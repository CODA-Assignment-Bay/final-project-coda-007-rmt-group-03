...
# 1️⃣ Load student_clusters parquet
df_clusters = spark.read.parquet("/opt/airflow/data/processed/student_clusters.parquet")

# 2️⃣ Group by student_id dan hitung rata-rata semua *_avg
...
# 3️⃣ Tentukan sub_cluster berdasarkan nilai tertinggi
expr_case = "CASE " + " ".join(
...
# 4️⃣ Tentukan cluster dari sub_cluster
sub_to_cluster = {}
...
df_students = df_students.withColumn("hobbies", from_json(col("hobbies"), array_schema)) \
                         .withColumn("extracurricular", from_json(col("extracurricular"), array_schema)) \
                         .withColumn("career_interest", from_json(col("career_interest"), array_schema))

# 7️⃣ Join
df_final = df_avg.join(df_students, on="student_id", how="left")
